    ...
    if request.method == "POST":
        ...
        try:
            ...
            try:
                ...
                post_params = get_params(transaction, currency, policy)
...
def dashboard(request):
    ...
    try:
        user_partner = Partner.objects.get(
            django_user=request.user.id)
        logger.info(
            "Partner: %s logged into system" % user_partner)
        return account(request)
    except ObjectDoesNotExist:
        pass

    insurance_policy_info = Policy.objects.order_by('-id').filter(
    ...
    try:
        found_fee_values = insurance_policy_info.values('fee')
    except Exception:
    ...
    policy_numbers = []
    try:
        found_policy_numbers = insurance_policy_info.values('id')
    ...
    try:
        found_start_dates = insurance_policy_info.values('start_date')
    except KeyError as error:
    ...
    try:
        found_expiration_dates = insurance_policy_info.values(
            'expiration_date')
    except KeyError as error:
    ...
    try:
        found_limits_of_liability = insurance_policy_info.values('cover_btc')
    except KeyError as error:
    ...
    try:
        found_dates_of_formatting = insurance_policy_info.values(
            'request_date')
    except KeyError as error:
    ...
    try:
        found_stock_exchanges = insurance_policy_info.values('exchange')
    except KeyError as error:
    ...
    try:
        found_policy_statuses = insurance_policy_info.values('status')
    except KeyError as error:
    ...
    context_policy = []
    for current_id, policy_id in enumerate(insurance_policy_info):
        ...
        try:
            ...
            logger.error(
                "An error has occurred while trying to get policy number.\
                Reason: " + str(error))
        ...
        try:
            ...
            logger.error(
                "An error has occurred while trying to get fee. Reason: " +
                str(error))
        ...
        try:
            ...
            if policy_id.payment_id and policy_id.payment_id.status < 0:
                ...
                logger.error(
                    "An error has occurred while trying to get insurance period.\
                    Reason: " + str(error))
        ...
        try:
            ...
            logger.error(
                "An error has occurred while trying to get limit . Reason: " +
                str(error))
        ...
        try:
            ...
            logger.error(
                "An error has occurred while trying to get date of formatting.\
                Reason: " + str(error))
        ...
        try:
            ...
            logger.error(
                "An error has occurred while trying to get exchange tag.\
                Reason: " + str(error))
        ...
        try:
            ...
            logger.error(
                "An error has occurred while trying to get policy status.\
                Reason: " + str(error))

        sos = False
        try:
            sos_exists = InsuranceCase.objects.filter(
                insurance=(policy_numbers[current_id])['id'])
            logger.debug(sos_exists.count())
            if sos_exists.count() > 0:
                sos = True
        except (IndexError, KeyError) as error:
            logger.error(
                "An error has occurred while trying to get InsuranceCase.\
                Reason: " + str(error))
        ...
        context_policy.append(context_policies)
    ...
    for stock_exchange in stock_exchange_tags:
        ...
        for policy in context_policy:
            if policy['stock'] == current_stock_exchange and (
                    policy['num_status'] == 1
                    or policy['num_status'] == 2):
